...
from mxm.moneymachine.marketdata.datasets.statistics_1d.coverage import (
    DayRange,
    complete_from_expected_and_observed,
)
from mxm.moneymachine.marketdata.datasets.statistics_1d.expected import (
    derive_expected_window,
)
from mxm.moneymachine.marketdata.datasets.statistics_1d.state import (
    BudgetContext,
    RetryPolicy,
    decide_action,
    derive_state,
...
from mxm.moneymachine.marketdata.vendors.databento.normalize.statistics_1d import (
...
@dataclass
class Statistics1DOrchestratorReport:
    product_id: str
    mode: Mode
    ts_utc: str
...
def _cov_snapshot(coverage: StoreCoverageSnapshot) -> AttemptsCoverageSnapshot:
    ...
    return AttemptsCoverageSnapshot(
        ...
        stats_path=(
            str(coverage.stats_path)
            if getattr(coverage, "stats_path", None) is not None
            else None
        ),
    )
...
def ingest_statistics_1d_for_product(
    *,
    ...
    contracts = _prepare_statistics_1d_contracts(
        product_id=product_id,
        mode=mode,
    ...
    for contract in contracts:
        ...
        _process_statistics_1d_contract(
            backend=backend,
            store=store,
            product_id=product_id,
            mode=mode,
    ...
    _finalize_statistics_1d_report(report)
    return report


def _process_statistics_1d_contract(
    *,
    backend: SQLiteBackend,
    store: Statistics1DStore,
    product_id: str,
    ...
    try:
        ...
        _populate_statistics_1d_expected_window(
            product_id=product_id,
            context=context,
            contract_context=contract_context,
        )
        ...
        _apply_statistics_1d_reset_if_requested(
            store=store,
            dry_run=dry_run,
            force_reset=force_reset,
            contract_context=contract_context,
...
def _init_statistics_1d_report(
    *,
    product_id: str,
    mode: Mode,
    cost_cap_usd: float,
) -> Statistics1DOrchestratorReport:
    return Statistics1DOrchestratorReport(
        product_id=product_id,
        mode=mode,
...
def _prepare_statistics_1d_run_context(
    *,
    ...
    if report.dataset_range_start is None or report.dataset_range_end is None:
        raise RuntimeError("statistics_1d dataset range gate did not populate range")

    avail_start_ts = to_utc_ts(parse_ts(report.dataset_range_start))
...
def _prepare_statistics_1d_contracts(
    *,
    product_id: str,
    mode: Mode,
...
def _populate_statistics_1d_interest_window(
    contract_context: Statistics1DContractContext,
) -> None:
...
def _require_statistics_1d_expected_window(
    contract_context: Statistics1DContractContext,
) -> Any:
...
def _populate_statistics_1d_coverage_before(
    *,
    store: Statistics1DStore,
    dry_run: bool,
    force_reset: bool,
...
def _decide_statistics_1d_contract_action(
    *,
    context: Statistics1DRunContext,
    force_reset: bool,
    contract_context: Statistics1DContractContext,
    ...
    _apply_statistics_1d_decision_status(
        decision=decision,
...
def _apply_statistics_1d_decision_status(
    *,
    decision: Any,
    derived_state: Any,
    contract_context: Statistics1DContractContext,
    ...
    if decision.action == "noop":
        _apply_statistics_1d_noop_status(
            derived_state=derived_state,
            decision=decision,
            contract_context=contract_context,
        )
...
def _apply_statistics_1d_noop_status(
    *,
    derived_state: Any,
    decision: Any,
    contract_context: Statistics1DContractContext,
...
def _execute_statistics_1d_contract_decision(
    *,
    client: Any,
    store: Statistics1DStore,
    dry_run: bool,
    ...
    _vendor_pull_normalize_write_statistics_1d(
        client=client,
        store=store,
        force_reset=force_reset,
        report=report,
...
def _vendor_pull_normalize_write_statistics_1d(
    *,
    client: Any,
    store: Statistics1DStore,
    force_reset: bool,
    ...
    df_raw = pull_statistics_1d_by_instrument_id(
        dataset=ident.dataset,
        instrument_id=ident.instrument_id,
        start=start,
        end=end,
...
def _ensure_statistics_1d_expected_window(
    *,
    product_id: str,
    context: Statistics1DRunContext,
    contract_context: Statistics1DContractContext,
...
def _append_statistics_1d_contract_run(
    *,
    report: Statistics1DOrchestratorReport,
    contract_context: Statistics1DContractContext,
) -> None:
